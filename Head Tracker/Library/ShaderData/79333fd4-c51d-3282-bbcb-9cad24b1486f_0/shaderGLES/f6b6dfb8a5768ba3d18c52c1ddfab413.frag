...
void main()
{
    ...
    vec3 srcColor = background.xyz;
    vec3 sucaiColor = eye.xyz;
    vec3 param = srcColor;
    vec3 param_1 = sucaiColor;
    float param_2 = eye.w;
    vec3 result = ApplyBlendMode(param, param_1, param_2);
    if (_ReflectionEnable > 0.5)
    {
        ...
        eyeReflection.w *= (mask * _ReflectionIntensity);
        vec3 reflectColor = eyeReflection.xyz;
        vec3 param_3 = result;
